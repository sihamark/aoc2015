...
object Day12 {
    ...
    private fun sum(any: Any): Int = when (any) {
        is Number -> any.toInt()
        is List<*> -> any.sumOf { sum(it ?: 0) }
        is Map<*, *> -> any.entries.sumOf { sum(it.key ?: 0) + sum(it.value ?: 0) }
        else -> 0
    }

    private fun sumNotRed(any: Any): Int = when (any) {
        is Number -> any.toInt()
        is List<*> -> any.sumOf { sumNotRed(it ?: 0) }
        is Map<*, *> -> {
            if (any.values.any { it == "red" }) {
                0
            } else {
                any.entries.sumOf { sumNotRed(it.key ?: 0) + sumNotRed(it.value ?: 0) }
            }
        }
        else -> 0
    }
